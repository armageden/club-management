...
export const hardwareRepository = {
  ...
  async checkout(eventId: string, data: CheckoutHardwareRequest, checkedOutBy: string): Promise<HardwareCheckout> {
    ...
    try {
      ...
      // Decrease available quantity and update status if depleted
      await client.query(
  ...
  async returnHardware(eventId: string, data: ReturnHardwareRequest): Promise<{ checkout: HardwareCheckout; returnRecord: HardwareReturn }> {
    ...
    try {
      ...
      // Fetch the updated checkout record
      const updatedCheckout = await client.query(
        `SELECT * FROM hardware_checkouts WHERE id = $1`,
        [data.checkout_id]
      );

      await client.query('COMMIT');
      return { checkout: updatedCheckout.rows[0], returnRecord: returnResult.rows[0] };
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
    } finally {
